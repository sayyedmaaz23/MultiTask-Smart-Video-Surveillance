    ...
    while True:
        ...
        if s:
            ...
            if len(main_buffer) == 32:
                ...
                try:
                    stage2_3_queue.put(main_buffer.copy(), timeout=2)
                except Exception:
...
def _run_mil_and_vivit():
    if MILnet is None or RESnet is None or ViViT is None:
        print("[Stage-2] ERROR: One or more models not initialized. Skipping.")
        return

    print("[Stage-2] MILnet starting.")
    prob_ordered_frames, raw_scores = [], []

    # Drain queue in one pass
    clips = []
    while stage2_3_queue.qsize() > 0:
        try:
            clip = stage2_3_queue.get(timeout=1)
            if len(clip) == 32:
                clips.append(clip)
        except Empty:
            break
    ...
    print(f"[Stage-2] Processing {len(clips)} clip(s) with MIL + ResNet.")

    # ── Batched ResNet + MIL scoring ──────────────────────────────────────────
    with torch.no_grad():
        for idx, clip in enumerate(clips):
            print(f"[Stage-2] Clip {idx + 1}/{len(clips)}")
            try:
                batch = torch.stack([
    ...
    if not raw_scores:
        print("[Stage-2] No valid scores produced — skipping ViViT.")
        return
    ...
    if vivit_results:
        ...
        print("[Stage-2] WARNING: ViViT produced no results for this batch.")
...
def _final_output(vivit_results):
    ...
    if conf > config.VIVIT_CONF_THRESHOLD:
        ...
        print("Some anomaly is detected but ViViT is not confident enough to classify it.")
